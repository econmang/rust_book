...
// Methods are similar to functions, but they are defined within the context of a struct (or enum
// or trait) and their first param is always self
// They are defined using implementation blocks (impls)
impl Rectangle {
    fn area(&self) -> u32 {
...
fn main() {
    ...
    dbg!(&rect);
    println!("\nThe area of the rectangle is (using height and width as independent vars) is {} square pixels.", area_w_h(width1, height1));
    println!("The area of the rectangle using dimensions is {} square pixels.", area_dims(dims));
    println!("The area of the rectange using the Rectangle struct & a function is {} square pixels", area_rect(&rect));
    if rect.width() {
        println!("The area of the rectange using the Rectangle struct & a method is {} square pixels", rect.area());
        // dbg! macro prints output to standerr
        println!("\nNote: Methods utilize automatic referencing and dereferencing so p1.dist(&p2) is equivalent to (&p1).dist(&p2), etc.");
    }
}
